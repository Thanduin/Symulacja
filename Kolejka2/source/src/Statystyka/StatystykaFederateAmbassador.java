...
public class StatystykaFederateAmbassador extends NullFederateAmbassador
{
            ...
            throws FederateInternalError
    {
        ...
        if( interactionClass.equals(federate.zajmijStolikHandle) )
        {
            ...
            federate.statystyka.ilosc_stolikow--;


        }
        if (interactionClass.equals(federate.zwolnijStolikHandle))
        {
            builder.append(" (zwolnij_stolik" );
            federate.statystyka.ilosc_stolikow++;
            federate.statystyka.ilosc_obsluzonych++;
        }
        if (interactionClass.equals(federate.dodajCzasHandle)){
            builder.append(" (dodaj_czas)" );
            byte[] bytes = theParameters.get(federate.czasDodajParametrHandle);
            HLAinteger32BE dodany_czas = new HLA1516eInteger32BE();
            try{
                dodany_czas.decode(bytes);
            } catch (DecoderException e){
                e.printStackTrace();
            }
            federate.statystyka.laczny_czas += dodany_czas.getValue();
        }
